...
describe("ECDSA P384", () => {
  ...
  const p1_x_words = splitToWords(BigInt(p1_x), 64n, 6n);
  const p1_y_words = splitToWords(BigInt(p1_y), 64n, 6n);
  const p2_x_words = splitToWords(BigInt(p2_x), 64n, 6n);
  const p2_y_words = splitToWords(BigInt(p2_y), 64n, 6n);
